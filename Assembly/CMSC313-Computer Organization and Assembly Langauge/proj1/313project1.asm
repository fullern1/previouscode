...
;;; Problems: none found yet
	xor edx, edx
.lastchar:
	cmp eax, 1		; Check if last char
	je .compare		; if last char jump to compare
	add esi, 1		; if not last char: move to next char
	sub eax, 1		; dec number of chars by 1
	add edx, 1		; increase counter by 1
	jmp .lastchar	; return to loop

.compare:
	mov bh, [esi]	; 
	cmp bh, 0AH		; compare value to end of line (line feed) hex value
	je .remove		; if equal jump to remove
	jmp .cont1		; if not equal jump to cont1

.remove:
	add eax, edx 	; return eax to starting value
	mov esi, buf	; return esi to starting value
	sub eax, 1		; dec eax by 1 to ignore end of line (line feed) value
...
.loop:
	cmp eax, 0		; check if at end of line
	je .done		; jump to done if at end of line
...
;;;  print message informing # of each type
	...
	add esp, 48		; add 48 to convert to ASCII value
	mov [int2char], esp
	mov eax, SYSCALL_WRITE
	mov ebx, STDOUT
	mov ecx, int2char
	...
	add ebp, 48		; add 48 to convert to ASCII value
	mov [int2char], ebp
	mov eax, SYSCALL_WRITE
	mov ebx, STDOUT
	mov ecx, int2char
	...
	add edi, 48		; add 48 to convert to ASCII value
	mov [int2char], edi
	mov eax, SYSCALL_WRITE
	mov ebx, STDOUT
	mov ecx, int2char
