...
int List::RemoveFromStart()
{
    ...
    int current;
    current = currentNode->m_data;
    m_head->m_next = currentNode->m_next;
    delete currentNode;
    

    return current;  // Replace VALUE_TO_RETURN with actual value
}
...
unsigned int List::Size() const
{
  ...
  return listSize;  // Replace VALUE_TO_RETURN with actual value
}
